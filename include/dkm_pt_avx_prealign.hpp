#pragma once
#ifndef DKM_PT_AVX_PREALIGN_H
#define DKM_PT_AVX_PREALIGN_H
...
std::vector<uint32_t> calculate_clusters_pt_avx_prealign(const std::vector<std::array<T, N>>& data,
	...
	for (size_t t = 0; t < nthr; ++t) {
		...
		if constexpr (N == 2 && std::is_same_v<T, float>) {
			pthread_create(&threads[t], nullptr, &details::worker_closest_mean_avx_2d_float_prealign_pt<T, N>, &a);
		} else {
...
template <typename T, size_t N>
std::tuple<std::vector<std::array<T, N>>, std::vector<uint32_t>> kmeans_lloyd_pt_avx_prealign(
	...
	uint64_t count = 0;
	do {
		clusters = details::calculate_clusters_pt_avx_prealign<T, N>(data, means, parameters);
