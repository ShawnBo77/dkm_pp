#pragma once
#ifndef DKM_TP_V2_AVX_UNALIGN_H
#define DKM_TP_V2_AVX_UNALIGN_H
...
std::vector<uint32_t> calculate_clusters_tp_v2_avx_unalign(const std::vector<std::array<T, N>>& data,
	...
	for (size_t t = 0; t < num_tasks; ++t) {
		...
		if constexpr (N == 2 && std::is_same_v<T, float>) {
			pool.enqueue([&, t]() { worker_closest_mean_avx_2d_float_unalign<T, N>(&args[t]); });
		} else {
...
template <typename T, size_t N>
std::tuple<std::vector<std::array<T, N>>, std::vector<uint32_t>> kmeans_lloyd_tp_v2_avx_unalign(
	...
	uint64_t count = 0;
	do {
		clusters = details::calculate_clusters_tp_v2_avx_unalign<T, N>(data, means, parameters);
